...
public class S3SinkTask extends SinkTask {
	...
	private class PartitionWriter {
		...
		private PartitionWriter(TopicPartition tp, long firstOffset) throws IOException {
			...
			//20180201_1236_car2go_vehicles_madrid.json rbp
			SimpleDateFormat df = new SimpleDateFormat("yyyyMMdd");
			df.setTimeZone(S3Writer.UTC);
			String name= df.format(new Date())+"car2go_vehicles_madrid";
			log.info("New temp file: {}/{} @ {}", path, name, firstOffset);
		...
		public void done() {
			...
			try {
				...
				s3.putChunk(writer.getDataFilePath(), tp);
			} catch (IOException e) {
